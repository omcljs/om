...
(deftest test-expr->ast
  ...
  (is (= (parser/expr->ast [:foo 0])
         {:type :prop :key [:foo 0] :dkey :foo :params {:root/id 0}}))
  (is (= (parser/expr->ast {:foo [:bar]})
         {:type :prop :key :foo :dkey :foo :sel [:bar]}))
  (is (= (parser/expr->ast {[:foo 0] [:bar]})
          {:type :prop :key [:foo 0] :dkey :foo :params {:root/id 0} :sel [:bar]}))
  ...
  (is (= (parser/expr->ast '({[:foo 0] [:bar :baz]} {:woz 1}))
         {:type :prop :key [:foo 0] :dkey :foo :sel [:bar :baz] :params {:root/id 0 :woz 1}}))
...
(deftest test-ast->expr
  (is (= (parser/ast->expr {:type :prop :key :foo :dkey :foo})
         :foo))
  (is (= (parser/ast->expr {:type :prop :key [:foo 0] :dkey :foo :params {:root/id 0}})
         [:foo 0]))
  (is (= (parser/ast->expr {:type :prop :key :foo :dkey :foo :sel [:bar]})
         {:foo [:bar]}))
  (is (= (parser/ast->expr {:type :prop :key [:foo 0] :dkey :foo :params {:root/id 0} :sel [:bar]})
         {[:foo 0] [:bar]}))
  ...
  (is (= (parser/ast->expr {:type :prop :key :foo :dkey :foo :sel [:bar :baz] :params {:woz 1}})
         '({:foo [:bar :baz]} {:woz 1})))
  (is (= (parser/ast->expr {:type :prop :key [:foo 0] :dkey :foo :sel [:bar :baz] :params {:root/id 0 :woz 1}})
         '({[:foo 0] [:bar :baz]} {:woz 1})))
